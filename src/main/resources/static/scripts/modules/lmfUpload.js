...
(function (factory) {
    ...
    var progress = {
        ...
        getTpl: function (files) {
            var LoopTpl = '   <tr class="template-upload file-no-{i} {hideClass}">' +
                '                <td class="preview">' +
                '                    <img src="{src}" onerror="this.src=\'http://dkongjian.dkongjian.cn/system/images/qiniu-image-error.jpg?201712251314\'">' +
                '                </td>' +
                '                <td class="size">' +
                '                    <p> &nbsp;</p>' +
                '                    <div class="progress">' +
    ...
    var getFileExt = function (base64) {
        ...
        if (base64.split(';base64,')[0].indexOf("data:image/") == -1) {
            return base64.split(';base64,')[0].split('/')[1];
        }
        ...
        uploadImgDataByObject: function (submitData, options) {
            ...
            if (fileBase64s.length === 0) {
                ...
                options.onFinish = function (successData) {
                    ...
                    $.isFunction(originSuccess) && originSuccess(finalData)
                    dfd.resolve(finalData);
                };
                options.onError = function (error) {
                    $.isFunction(originError) && originError(error);
                    dfd.reject(error);
                };
