...
layui.use(['form', 'layer', 'lmfEvent', 'laytpl', 'chooseRole', 'chooseUser', 'chooseDepartment', 'chooseCombination'], function () {
    ...
    form.on('submit(form-submit)', function (obj) {
        ...
        if (workflow_step_list.length < 1) {
            layer.Notify.error('审核流必须配置审核步骤!');
            return false
        }
        ...
        $.ajax(
            ...
            .then(
                function (res) {
                    ...
                    layer.Notify.success('编辑审核流成功!');
                    location.href = "/workflow/layout/index.html";
                }
            )
            .fail(function (e) {
                layer.close(index);
                layer.Notify.error('编辑审核流失败!');
            });
        return false;
    });

    var contactsCount = workflowSteps.length;
    ...
    clickEvent('change-approval-group', function (e) {
        var $stpItem = $(this).parents(".step-item");
        var approvalGroupType = $stpItem.find('[lay-filter="approval_group_type_filter"]').val();
